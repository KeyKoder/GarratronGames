var socket;
var terrain, nightTerrain, player, map;
var tileSize=32;
var dayTime=true;
function preload() {
...
var rocks=[], logs=[], minerals=[], portals=[];
...
function draw(){
	...
	for(var r=0;r<layer0.length;r++){
		for(var t=0;t<layer0[r].length;t++){
			image(dayTime ? terrain : nightTerrain,t*tileSize,r*tileSize,tileSize,tileSize,layer0[r][t]*tileSize,0,tileSize,tileSize);
		}
	}

	for(var r=0;r<layer1[player.mapId].length;r++){
		for(var t=0;t<layer1[player.mapId][r].length;t++){
			image(dayTime ? terrain : nightTerrain,t*tileSize,r*tileSize,tileSize,tileSize,layer1[player.mapId][r][t]*tileSize,0,tileSize,tileSize);
		}
	}
	...
	for(var r=0;r<layer2[player.mapId].length;r++){
		for(var t=0;t<layer2[player.mapId][r].length;t++){
			if(layer2[player.mapId][r][t]==8 && minerals.length<layer2[player.mapId].count2d(8))
				minerals.push(new Diamond(t, r, true));
		}
	}
	...
	for(var r=0;r<layer3[player.mapId].length;r++){
		for(var t=0;t<layer3[player.mapId][r].length;t++){
			...
			image(dayTime ? terrain : nightTerrain,t*tileSize,r*tileSize,tileSize,tileSize,layer3[player.mapId][r][t]*tileSize,64,tileSize,tileSize);
			//tint(255);
		}
	}
