function Item(img, x, y, ix, iy, id){
	this.img=img;
	this.x=x*tileSize+8;
	this.y=y*tileSize+8;
	this.ix=ix;
	...
	this.update = function() {
		...
		if((player.x>this.x-10 && player.x<this.x+10) && (player.y>this.y-10 && player.y<this.y+10)){
			...
			player.inventory.pickup(this.id);
		}
	}
}
...
function Rock(x,y,semibroke){
	...
	this.show = function(){
		if(!this.broken)
			this.brokenPercent>=2.5 ? image(dayTime ? terrain : nightTerrain,this.x,this.y,32,32,64,32,32,32) : image(dayTime ? terrain : nightTerrain,this.x,this.y,32,32,32,32,32,32);
		else{
			if(!this.droppedItem){
				items.push(new Item(dayTime ? terrain : nightTerrain, x, y, 32, 32, "stone"));
	...
	this.show = function(){
		if(!this.broken){
			if(this.brokenPercent>-0.1 && this.brokenPercent<0.5){
				image(dayTime ? terrain : nightTerrain,this.x,this.y,32,32,64+32,32,32,32);
			}else if(this.brokenPercent>0.4 && this.brokenPercent<1){
				image(dayTime ? terrain : nightTerrain,this.x,this.y,32,32,64+64,32,32,32);
			}else if(this.brokenPercent>0.9 && this.brokenPercent<1.5){
				image(dayTime ? terrain : nightTerrain,this.x,this.y,32,32,64+64+32,32,32,32);
			}else if(this.brokenPercent>1.4 && this.brokenPercent<2){
				image(dayTime ? terrain : nightTerrain,this.x,this.y,32,32,64+64+64,32,32,32);
			}
		}else{
			if(!this.droppedItem){
				items.push(new Item(dayTime ? terrain : nightTerrain, x, y, 64+32, 32, "wood"));
				this.droppedItem=true;
			}
		}
	}
...
function Iron(x,y,hasMineral){
	...
	this.show = function(){
		if(!this.broken){
			image(dayTime ? terrain : nightTerrain,this.x,this.y,32,32,64*4,32,32,32);
		}else{
			if(!this.droppedItem){
				items.push(new Item(dayTime ? terrain : nightTerrain, x, y, 64*4, 32, "iron"));
				this.droppedItem=true;
			}
		}
	}
